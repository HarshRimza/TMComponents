...
import tkinter.font
class User:
 def __init__(self,name,accept=None,decline=None):
  self.name=name
  self.accept=accept
...
class DataModel():
 def __init__(self):
  ...
  self.acceptIcon=tkinter.PhotoImage(file="accept.png")
  self.declineIcon=tkinter.PhotoImage(file="decline.png")
 ...
 def getValueAt(self,rowIndex,columnIndex,master=None):
  ...
  if columnIndex==1:
   return u.name
  if columnIndex==2: 
   if u.accept is None: u.accept=tkinter.Button(master,image=self.acceptIcon,width=24,height=24)
   return u.accept
  if columnIndex==3:
   if u.decline is None: u.decline=tkinter.Button(master,image=self.declineIcon,width=24,height=24)
   return u.decline
 def getCellType(self,rowIndex,columnIndex):
  if columnIndex==0: return "int"
  if columnIndex==1: return "str"
  return "Button"
 def getContentAlignment(self,rowIndex,columnIndex):
  if columnIndex==0: return "right"
  if columnIndex==2 or columnIndex==3: return "center"
  return "left"
 def getCellCommand(self,rowIndex,columnIndex):
  if columnIndex==2 or columnIndex==3: return self.editCommand
  return None
...
titleFont=tkinter.font.Font(family="Comic Sans MS",size=16,weight="bold")
dataFont=tkinter.font.Font(family="Comic Sans MS",size=14)
tmGrid=TMGrid(window,model,610,600,40,titleFont,dataFont)
