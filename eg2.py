...
import tkinter.font
class Student:
 def __init__(self,rollNumber,name,gender,button=None):
...
class DataModel():
 def __init__(self):
  ...
  self.image=tkinter.PhotoImage(file="edit.png")
 ...
 def getValueAt(self,rowIndex,columnIndex,master=None):
  ...
  if s.button is None: s.button=tkinter.Button(master,image=self.image,width=24,height=24)
  return s.button
  ...
  for r in range(rows):
   ...
   for c in range(columns):
    ...
    if cellType=="PhotoImage":
     ...
     self.create_window(x+buttonX,y+buttonY,anchor='nw',window=button)
    else:
     cellContent=self.model.getValueAt(r,c)
     contentWidth=dataFont.measure(cellContent)
     contentHeight=dataFont.metrics("linespace")
...
def editClicked(rowIndex,columnIndex):
 ...
 model.data[rowIndex].button.config(state="disabled")
